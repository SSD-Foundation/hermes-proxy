...
- **Key management**: A file-based keystore (Argon2id-derived master key via Argon2id + XChaCha20-Poly1305) stores node identity material (`mesh.identity_secret`) and versioned per-chat records (chat_id, key_version, X25519 pub/priv, HKDF salt/info, derived send/recv/mac/ratchet seeds, legacy combined blobs migrated forward). Secrets are sealed with tamper detection and zeroed on overwrite/delete; chat participant and remote ephemeral keys are zeroed when chats end or expire. Crypto knobs (HKDF hash/info label, max key lifetime) are validated at startup.
